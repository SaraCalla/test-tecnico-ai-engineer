...
def embed_chunks(chunks):
    ...
    for i in range(0, len(texts), settings.embedding_batch_size):
        ...
        logger.info(f"  Embedded {end}/{len(texts)} chunks")
...
def store_chunks(chunks):
    ...
    logger.info(f"  Stored {len(chunks)} chunks in '{settings.qdrant_collection_name}'")


def main():
    logger.info("=== Step 1: Parsing menus ===")
    menus = parse_all_menus()
    total_dishes = sum(len(m.dishes) for m in menus)
    logger.info(f"  {len(menus)} menus, {total_dishes} dishes\n")
    ...
    logger.info("=== Step 4: Storing in Qdrant ===") # Docker is used to run Qdrant
    store_chunks(chunks)
